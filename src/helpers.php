...
if (!function_exists('storage_path')) {
     ...
     * 获取存储目录 (storage/) 路径
     * 
     * 返回 storage 目录的绝对路径，用于存储应用生成的文件。
     * 常用于存储日志、缓存、上传文件等。
     * 
     * @param string $path 要拼接的子路径
     * @return string 完整的绝对路径
     ...
     * ```php
     * // 获取 storage 目录
     * $storage = storage_path();  // /var/www/app/storage
     * 
     * // 获取日志文件路径
     * $log = storage_path('logs/app.log');  // /var/www/app/storage/logs/app.log
     * ```
     */
...
if (!function_exists('logger')) {
     ...
     * ```
     */
    function logger(): \HolySword\Log\Logger
    {
        static $logger = null;
        if ($logger === null) {
            $logger = new \HolySword\Log\Logger(storage_path('logs'));
        }
        return $logger;
    }
}
